...
function AddExpenseModal({ onClose, onSuccess }) {
  ...
  const [date, setDate] = useState(getCurrentMonth()); // Default to current month
  const [amount, setAmount] = useState('');
  const [category, setCategory] = useState('');
  const [customCategory, setCustomCategory] = useState('');
  const [message, setMessage] = useState('');
  const [isRecurring, setIsRecurring] = useState('');
  const [showSessionExpired, setShowSessionExpired] = useState(false); // Flag for session expiry UI

  // Get user context values and functions
...
const handleSubmit = async (e) => {
  ...
  try {
    // Call authenticated fetch helper with POST request to add expense
    ...
    if (res.ok) {
      ...
      onSuccess(); // Trigger success callback (e.g. refresh data)
